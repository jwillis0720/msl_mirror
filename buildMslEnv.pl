...
foreach $lib (keys %libs){
	...
	system("cd ./$name"."_src/$fname; ./bootstrap.sh --prefix=`pwd`/../../$name"."_libs/ --with-libraries=iostreams,serialization,regex");
	system("cd ./$name"."_src/$fname; ./b2");
	system("cd ./$name"."_src/$fname; cp ./stage/lib/libboost_*.a ../../$name"."_libs/");
	system("cd ./$name"."_src/$fname; cp -r boost ../../$name"."_incs/");
    }
